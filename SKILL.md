...
**以下规则没有任何例外、没有任何借口、不可通过任何推理绕过：**

### 规矩 1: 必须搜完规定的条数
- 选高强度 → 必须执行全部 600+ 条，少 1 条都不行
- 选中强度 → 必须执行约 300 条，不得低于 280 条
- 选低强度 → 必须执行约 150 条，不得低于 130 条
- **禁止以下借口**: "信息已足够""结果重复了""时间太长""上下文要满了""用户可能等不及"

### 规矩 2: 必须覆盖全部 10 个维度
- 即使某个维度前几条搜索无结果，也必须执行完该维度的全部配额
- **禁止**: "这个维度搜不到东西，跳过算了"

### 规矩 3: 搜索 = 机械执行，不是智能筛选
- 你是一个**搜索执行器**，不是搜索决策者
- 你要做的: 取出下一条查询模板 → 填入学校专业名 → 调用 WebSearch → 记录结果 → 取下一条
- **禁止**: 在搜索过程中评估查询质量并"优化"掉你认为不必要的查询

### 规矩 4: 必须逐批报告进度
- 每完成 1 批（8-10条），必须输出: `[进度] 已完成 X/总数 (Y%) | 当前维度: ZZZ | 本批新增 N 条 Claim`
- 这个数字是**硬证据**——如果数字对不上，说明你跳过了搜索

### 规矩 5: 搜索不完成，禁止进入 Phase 3
- 搜索条数没有达到目标数之前，**禁止开始任何分析**
- **禁止**: "边搜边分析，搜到一半就开始写报告"
...
- **死规矩优先**: 以上 5 条死规矩优先于所有其他考虑（包括效率、时间、Token 预算）
...
用户选择后，明确声明: "已锁定 {强度} 模式，目标 {N} 条搜索，覆盖全部 10 维度。开始执行。"

### Phase 1: 列出全部待搜索查询（必须执行，不可跳过）

根据所选强度，将下方查询模板代入 `{school}` 和 `{major}`，**显式列出一个带编号的完整清单**（1, 2, 3, ... N）。
这张清单就是你的执行列表——每完成一条，从清单上划掉一条。

| 强度 | 查询数量 | 选取规则 |
|------|---------|---------|
| 🔥 高 | 全部 600+ 条 | 逐条列出，编号 1-600+ |
| ⭐ 中 | ~300 条 | 列出所有子节名称，每节选前 50-60% 的查询，编号 1-300 |
| 💡 低 | ~150 条 | 列出所有子节名称，每节选核心 3-5 条，编号 1-150 |

**列清单时注意**:
- 每个维度分配必须符合该强度的配额（见 Phase 0 的数据）
- 社媒维度（知乎/贴吧/小红书/B站）不能低于配额
- 清单末尾标注总数，这个数字就是你的硬指标

### Phase 2: 机械执行搜索（禁止思考，只许执行）

你的角色: **搜索执行器**。从一个编号清单中逐条取出查询，调用 WebSearch，记录结果，取下一条。

**操作步骤（严格按此循环，不要加入自己的判断）**:

```
循环:
  1. 从 Phase 1 生成的清单中，按顺序取下一批（每批 8-10 条）
  2. 调用 WebSearch 执行这批查询（不要判断"这条有没有用"）
  3. 将原始结果追加保存到 reports/raw_search_{school}_{major}_batch{N}.json
  4. 从本批搜索结果摘要中提取 Claim，追加到 reports/claims_{school}_{major}.json
  5. 从清单上划掉已完成的条目
  6. 输出进度: [进度] 已完成 X/总数 (Y%) | 当前维度: ZZZ | 本批新增 N 条 Claim | 累计 Claim: M 条
  7. 如果清单还有剩余条目，回到步骤 1
  8. 如果清单全部完成，输出 "搜索完成: X/总数 (100%)" 并进入 Phase 3
```

**禁止行为清单**:
- ❌ 禁止查看搜索结果后说"够了"并提前停止
- ❌ 禁止跳过清单上任何一条查询（即使你认为它和前面的重复）
- ❌ 禁止在一个维度结果"足够好"后跳到下一个维度
- ❌ 禁止合并"相似"查询（清单上列了多少条就执行多少条）
- ❌ 禁止在搜索过程中启动分析
- ❌ 禁止以"Token 预算""时间效率""上下文限制"为由削减搜索

**错误处理**:
- 某条搜索无结果 → 记录为 `{query: "...", results: [], status: "no_results"}`，继续下一条
- 某条搜索 400 错误 → 按内容安全过滤策略换词重试最多 2 次，仍失败则记录并继续
- 某批全部失败 → 记录失败原因，**不跳过剩余查询**，继续下一批
...
1. **我是一个执行器，不是决策者** — 清单上写了什么就执行什么
2. **条数就是法律** — 高强度 600/中强度 300/低强度 150，差一条都不算完
3. **10 维度一个不能少** — 即使某维度搜不到结果也要搜完配额
4. **搜完才能分析** — 搜索 100% 完成是进入 Phase 3 的唯一前提
5. **每批报数** — `[进度] X/Y (Z%)`，数字是最诚实的

**自检**: 如果发现自己在想"这个维度已经够了，跳过去吧"——停，这是违规的。继续搜。
4. 搜索 JSON 结构：
```json
{
  "school": "{school}",
